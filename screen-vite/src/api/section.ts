...
import {SecInfoElem, SectionData, SecTimeRangeElem} from "@/model/module.ts";
 ...
 * @return {SectionData} 成功时,返回断面数据
 * @return {HttpErrorData} 失败时,返回错误信息
 */
export async function getTodaySecDataAPI(time:number):Promise<SectionData|HttpErrorData>{
    return await httpGet<SectionData>(API.GetTodaySecData,
        {
            selectTime: time,
        }
    );
 ...
 * @return {HttpErrorData} 失败时,返回错误信息
 */
export async function getSpecificDaySecDataAPI(date:string, time:number):Promise<SectionData|HttpErrorData>{
    return await httpGet<SectionData>(API.GetSpecificDaySecData,
        {
            date: date,
            selectTime: time,
        }
 ...
 * @return {HttpErrorData} 失败时,返回错误信息
 */
export async function getLastedSecSpeedDataAPI():Promise<SectionData|HttpErrorData>{
    return await httpGet<SectionData>(API.GetLastedSecSpeed);
}
