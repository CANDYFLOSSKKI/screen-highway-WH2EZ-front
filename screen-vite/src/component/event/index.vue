  ...
  <div class="xpanel-wrapper xpanel-wrapper-45">
    <div class="xpanel xpanel-r-b">
      ...
      <table style="height: 440px;">
        ...
        <tr v-for="data in carEventTable" @click="alertCarEvent(data)" class="no-select" style="font-size: 20px;">
          <td>{{ data.time }}</td>
          <td>{{ data.id }}</td>
          <td>{{ data.location }}</td>
...
import {EventData, EventEntity} from "@/model/module.ts";
...
async function getTodayCarEventData() {
  ...
  if (!res) { return; }
  inputTime.value = (res as EventData).day;
}
 ...
 */
function alertCarEvent(data:EventEntity) {
