...
import {SectionData, SectionTableData} from "@/model/module.ts";
...
// 断面速度数据
const sectionSpeedData = ref<Array<SectionTableData>>([]);
 ...
 * @param data 断面数据
 */
function updateSectionSpeedData(data:SectionData) {
 ...
 * @param data 断面数据
 */
function combineSectionSpeedData(data:SectionData) {
  // 合并不同断面相同时间的数据
  for (let elem of selectSecOptions.value) {
    sectionSpeedData.value.push({
      name: elem.label,
